...
use stellar_xdr::curr::{self as xdr, BucketEntry, Frame, Limited, Limits, ReadXdr};
...
impl HistoryArchiveClient {
    ...
    pub async fn get_bucket(&mut self, bucket_hash: &str) -> Result<Vec<Frame<BucketEntry>>> {
        ...
        self.bucket_cache
            .insert(bucket_hash.to_string(), entries.clone());
        Ok(entries)
    }
    ...
    async fn get_history(&mut self, ledger: Option<u32>) -> Result<History> {
        ...
        if let Some(ledger) = ledger {
            ...
            self.history_cache
                .insert(history.current_ledger, history.clone());
        }
    ...
    fn build_history_url(&self, ledger: Option<u32>) -> Result<Url> {
        ...
        let history_path = if let Some(ledger) = ledger {
            ...
            format!(
                ...
                ledger_hex_0, ledger_hex_1, ledger_hex_2, ledger_hex
            )
        } else {
